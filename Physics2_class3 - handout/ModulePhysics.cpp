...
bool PhysBody::Contains(int x, int y) const
{
	...
	for (b2Fixture* Fixture = body->GetFixtureList(); Fixture && !ret; Fixture = Fixture->GetNext())
	{
		
		ret = Fixture->GetShape()->TestPoint(Fixture->GetBody()->GetTransform(), vec);
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	for (b2Fixture* Fixture = body->GetFixtureList(); Fixture; Fixture = Fixture->GetNext())
	{
		...
		if (Fixture->RayCast(output, input, 0))
		{
			...
			ret = (sqrt(((x2 - x1) * (x2 - x1)) + ((y2 - y1) * (y2 - y1))) * output->fraction); //int distance_collision
		}
	}
